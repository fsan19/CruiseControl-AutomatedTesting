...
def fullTest(newFileName,outputFileName,desiredOutputFile):
    ...
    p = subprocess.Popen(["sh", workingDir +'/autoTest.sh'], stdin=PIPE, stdout=PIPE)
    ...
    for i, entry in enumerate(programOutputList):
        ...
        for headerName in outputHeaderList:
            ...
            if (programOutputList[i][headerName] != expectedOutputList[i][headerName]):
                isFail = True
                    
        if isFail:
            print(("\033[1;31;40m Failed! Test # {}".format(testCnt)))
            print("\033[1;37;40m Expected Outputs: " + str(expectedOutputList[i]))
            print("\033[1;31;40m Actual Outputs: "+ str(programOutputList[i]) + '\n\n')
            break
        else:
            print("\033[1;32;40m Passed! Test #"+ str(testCnt))
            print("\033[1;37;40m Outputs: "+ str(programOutputList[i]) + '\n')
            testCnt += 1
...
def Test(fileName,newFileName,outputFileName,desiredOutputFile):
    ...
    if user =="1":
        ...
        workingDir = os.getcwd()
        p = subprocess.Popen(["sh", workingDir +'/autoTest.sh'], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        c=""
        output = []
